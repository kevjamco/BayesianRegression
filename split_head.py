...
import numpy as np
from models.BNN_split_head import BayesianSplit
...
# pyRidge code
rig = BayesianSplit(num_iters=10000, alpha=0.04, beta=0.5, deg=6, normalization=False)
...
for sample in range(samples):
    pred, sig_pred = rig.predict(X_new, fixed=False)
    y_predict[sample,:] = pred[:,0]

y_predict_fixed, sig_pred_fixed = rig.predict(X_new, fixed=True)
...
# plot heteroscedastic aleatoric uncertainty
plt.plot(X_new, sig_pred_fixed, "b-")
